        ...
        if commande('mc_serv'):
            if len(message.content.split()) >= 2:
                if message.content.split()[1] == 'show':
                    serversList = openFile(mainFiles,'servers_minecraft','r')
                    ...
                    if len(message.content.split()) >= 4:
                        ...
                        try:
                            ...
                            cache = openFile(mainFiles,'servers_minecraft','r')
                            cache.append({'name':message.content.split()[2],'ip':message.content.split()[3]})
                            openFile(mainFiles,'servers_minecraft','w',cache)
                    ...
                    if len(message.content.split()) >= 3:
                        cache = {}
                        cache['file'] = openFile(mainFiles,'servers_minecraft','r')
                        cache['bool'] = False
                        for i in range(len(cache['file'])):
                            if cache['file'][i]['name'] == message.content.split()[2]:
                                cache['bool'] = True
                        if cache['bool']:
                            await message.channel.send(f'Le serveur **{cache["file"][i]["name"]}** (`{cache["file"][i]["ip"]}`) à bien été supprimé.')
                            cache['file'].pop(i)
                            openFile(mainFiles,'servers_minecraft','w',cache['file'])
                        else:
